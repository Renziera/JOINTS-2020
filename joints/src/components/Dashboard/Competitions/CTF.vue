                ...
                <v-row>
                    ...
                    <v-col md="8" lg="8" xs="12" sm="12" class="col-xs-12">
                        ...
                        >
                            ...
                            <div class="subtitle">Kategori Kompetisi: </div>
                            ...
                            <div class="subtitle">Nama Perguruan Tinggi/Sekolah:</div>
                            <v-text-field
                                class="mt-1"
                                v-bind:disabled="this.isMemberKetua"
                                    ...
                                    >
                                        <v-card>
                                            ...
                                            <div class="pa-6 title font-weight-regular">
                                              Apakah Anda sudah yakin dengan data yang Anda masukkan dan bersedia mengikuti proses pendaftaran Kompetisi Capture The Flag JOINTS 2020?  
                                            </div>
...
export default {
    ...
    data() {
        return {
            myInterval : null,
            options: [
              { item: null, name: 'Silahkan pilih kategori kompetisi', disabled: true },
              { item: true, name: 'SMA' },
              { item: false, name: 'Universitas' },
            ],
    ...
    methods: {
        ...
        Axios.get(BASE_URL + '/competition', config)
            .then(response => {
              // console.log(response);
                ...
                if(competitions.ctf != undefined){
                  // console.log(competitions.ctf.status);
                // console.log('ini dari getstatusdata; ADA COMPETITION.ctf');
                  if (competitions.ctf.status == "menunggu_pembayaran" ) {
                      //  console.log('ga lunas menunggu pembayaran');
                      this.overlay = false;
                      this.isLunas = false;
                      this.isBelumBayar = true 

                  } else if (competition.ctf.status == "lunas") {
        ...
        if(this.$store.getters.competitions.ctf == null ||
            this.$store.getters.competitions.ctf  == undefined ){
            ...
            // console.log(this.$store.getters.competitions);
                ...
                this.team.nama_tim= this.$store.getters.competitions.ctf .nama_tim,
                this.team.nama_1= this.$store.getters.competitions.ctf .nama_1,
                this.team.nama_2 = this.$store.getters.competitions.ctf .nama_2,
                this.team.ktm_ketua = this.$store.getters.competitions.ctf .ktm_ketua,
                this.team.ktm_1= this.$store.getters.competitions.ctf .ktm_1,
                this.team.ktm_2= this.$store.getters.competitions.ctf .ktm_2,
                this.team.harga=  this.formatHarga(this.$store.getters.competitions.ctf .harga),
                this.team.status = this.$store.getters.competitions.ctf .status
                this.team.isSingle = this.$store.getters.competitions.ctf .single

                if(this.$store.getters.competitions.ctf .ktm_2 != undefined){
                      this.isMemberTwo = true;
                      this.isMemberOne = true;
                      this.isMemberKetua = true
                      this.jumlahAnggota = 3
                } else if (this.$store.getters.competitions.ctf .ktm_1 != undefined){
                      this.isMemberOne = true;
                      this.isMemberKetua = true
                      this.jumlahAnggota = 2
                } else if (this.$store.getters.competitions.ctf .ktm_ketua != undefined){
                      this.isMemberKetua = true;
                      this.jumlahAnggota = 1
                }

                if (
                      this.$store.getters.competitions.ctf .status == 'menunggu_pembayaran'
                  ) {
                      // console.log('ga lunas menunggu pembayaran');
                      this.overlay = false;
                      this.isLunas = false;
                      ...
                      this.$store.getters.competitions.ctf .status == 'lunas'
                  ) {
                      // console.log('lunas dari === lunas');
                      this.isLunas = true;
                      this.overlay = true;
